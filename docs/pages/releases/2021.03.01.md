...
It's been an epic *month* of problem-solving, but I finally got it. :D Highlight for this release is **Automatic Initial State**!
...
It may end up getting support for DG later on, the trouble lies in the callback we use to read an edited initial state is not reliable in DG. It's possible we'll find another means of achieving the same goal in the future, although DG really is part of the past and Ragdoll works best under Parallel for other reasons to, primarily performance but also stability and robustness.
