...
<a href="" class="button red"><b>Download Manikin</b></a>
</div>
...
You can now constrain one marker to another!
...
| Pin Constraint | Match a position and orientation in worldspace, similar to `Pose Space = World`.
...
Similar to the `Soft Pin` used with Rigids, this creates a new position and orientation a marker will try and reach. It's exactly what you get using `Pose Space = World` on the markers themselves, but with the convenience of a new transform you can animate. They will both try and pull on the marker, greatest stiffness wins!
...
All poses, limits and constraints now have their values combined with equivalent values on the solver node.
