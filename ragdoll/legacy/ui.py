...
def hide_menuitem(item):
    ...
    item = shiboken2.wrapInstance(internal.long(ptr), QtWidgets.QAction)
    ...
    if not self._maya_window:
        ...
        if ptr is not None:
            self._maya_window = shiboken2.wrapInstance(
                internal.long(ptr), QtWidgets.QMainWindow
            )
...
class Explorer(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def reload(self):
        ...
        if isinstance(dump, internal.string_types):
...
def view_to_pixmap(size=None):
    ...
    buf = buf.from_address(internal.long(image.pixels()))
